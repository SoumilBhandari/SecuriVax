  ...
  return (
    <section className="panel p-5">
      <div className="mb-2 flex items-baseline justify-between gap-2">
        <h2 className="m-0 text-[15px] font-semibold">Every confirmed VVM photo calibrates the model</h2>
        {data.photos > 0 && <span className="text-xs text-neutral-500">{data.photos} field photo{data.photos === 1 ? "" : "s"}</span>}
      </div>
      <p className="m-0 text-sm leading-[1.45] text-neutral-300">
      ...
      {learnt.length === 0 ? (
        <p className="tile m-0 mt-3 !p-3 text-sm text-neutral-400">
        ...
        <ul className="m-0 mt-3 list-none p-0">
          {learnt.map((p) => (
            <li key={p.product_id} className="border-t border-line py-2.5 first:border-0">
              <div className="flex items-baseline justify-between gap-2">
                <p className="m-0 truncate text-sm font-semibold">{p.name}</p>
...
function SpeedBar({ lo, mid, hi }: { lo: number; mid: number; hi: number }) {
  ...
  return (
    <div className="relative mt-1.5 h-3 rounded-full bg-neutral-800" aria-hidden>
      <div className="absolute inset-y-0 rounded-full bg-accent/30" style={{ left: x(lo), width: `calc(${x(hi)} - ${x(lo)})` }} />
      <div className="absolute -top-0.5 h-4 w-0.5 bg-neutral-300/60" style={{ left: x(1) }} />
      <div className="absolute top-0 h-3 w-3 -translate-x-1/2 rounded-full bg-accent-400" style={{ left: x(mid) }} />
    </div>
  );
}
