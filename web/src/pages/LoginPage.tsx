...
/** Staff sign-in: looking needs none; loading boxes, VVM checks and dispatch do. */
export default function LoginPage() {
  ...
  const submit = (e: FormEvent) => {
    e.preventDefault();
    void run(() => (mode === "signin" ? signIn(email, password) : signUp({ email, password, name, operator_code: code })));
  };
      ...
      <main className="flex flex-1 items-start justify-center px-6 pb-16 pt-6 sm:items-center sm:pt-0">
        <div className="modal-in w-full max-w-sm">
          <div className="mb-10 text-center">
            <p className="mono-label m-0">{signup ? "New staff account" : "Staff sign-in"}</p>
          ...
          <div className="mt-10 text-center">
            <Link to={next.split("?")[0]} className="mono-label no-underline hover:text-text">
